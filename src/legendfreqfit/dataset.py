    ...
    def __init__(
        ...
        for parameter in model_parameters:
            if parameter not in self.model.parameters:
                msg = (
                    f"`Dataset` `{self.name}`: model_parameter `{parameter}` not found in model `{self.model}`."
                )
                raise KeyError(msg)
        
        # check that all required parameters were passed
        for parameter, defaultvalue in self.model.parameters.items():
            if ((defaultvalue is str and defaultvalue == "nodefaultvalue") 
                and (parameter not in model_parameters)):
                msg = (
                    f"`Dataset` `{self.name}`: required model parameter `{parameter}` not found in model_parameters"
                )
                raise KeyError(msg)
        ...
        for i, (par, defaultvalue) in enumerate(self.model.parameters.items()):   
            ...
            # parameter was passed and should be included in the fit
            elif (("includeinfit" in parameters[model_parameters[par]]) and (parameters[model_parameters[par]]["includeinfit"])):      
                self.costfunction._parameters |= {
                    model_parameters[par]: 
                    parameters[model_parameters[par]]["limits"] if "limits" in parameters[model_parameters[par]] else None}
                ...
                if (("value" not in parameters[model_parameters[par]]) and (defaultvalue == "nodefaultvalue")):
                    msg = (
