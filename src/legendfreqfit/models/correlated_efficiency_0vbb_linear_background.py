...
def nb_pdf(
    ...
    pdf(E) = 1/(mu_S+mu_B) * [mu_S * norm(E_j, QBB + delta, sigma) + m*E + BI*exp]
    """
    ...
    mu_B = exp * BI * (WINDOW[-1][-1] - WINDOW[0][0]) + m / 2 * (
        WINDOW[-1][-1] ** 2 - WINDOW[0][0] ** 2
    )
...
def nb_extendedrvs(
    ...
    n_bkg = np.random.poisson(
        exp * BI * (WINDOW[-1][-1] - WINDOW[0][0])
        + m / 2 * (WINDOW[-1][-1] ** 2 - WINDOW[0][0] ** 2)
    )
    ...
    raise NotImplementedError(
        "The random draws for this distribution are not yet quite correct"
    )
...
class correlated_efficiency_0vbb_linear_background_gen:
    ...
    def pdf(
        ...
        return nb_pdf(
            Es, S, BI, delta, sigma, eff, effunc, effuncscale, exp, m, check_window
        )
    ...
    def logpdf(
        ...
        return nb_logpdf(Es, S, BI, delta, sigma, eff, effunc, effuncscale, exp, m)
    ...
    def density(
        ...
        return nb_density(Es, S, BI, delta, sigma, eff, effunc, effuncscale, exp, m)
    ...
    def density_gradient(
        ...
        return nb_density_gradient(
            Es, S, BI, delta, sigma, eff, effunc, effuncscale, exp, m
        )
    ...
    def log_density(
        ...
        if (mu_S + mu_B <= 0) or np.isnan(np.array([mu_S, mu_B])).any():
            ...
            return (
                mu_S + mu_B,
                np.log(mu_S + mu_B)
                + nb_logpdf(Es, S, BI, delta, sigma, eff, effunc, effuncscale, exp, m),
            )
    ...
    def rvs(
        ...
        return nb_rvs(n_sig, n_bkg, delta, sigma, m, seed=seed)
    ...
    def extendedrvs(
        ...
        return nb_extendedrvs(
            S, BI, delta, sigma, eff, effunc, effuncscale, exp, m, seed=seed
        )
    ...
    def plot(
        ...
        y = nb_pdf(Es, S, BI, delta, sigma, eff, effunc, effuncscale, exp, m)
