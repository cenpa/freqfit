...
def test_dataset_errors():
    ...
    with pytest.raises(KeyError):
        dset = Dataset(
            data, model, model_parameters_wrong, parameters, costfunction, name
        )
        dset = ToyDataset(
            model,
            model_parameters_wrong,
...
def test_toydataset():
    ...
    dset = ToyDataset(
        model, model_parameters, model, model_parameters, parameters, costfunction, name
    )
    ...
    S = 100
    BI = 100
    reference_density = gaussian_on_uniform.density(
...
def test_combined_dataset():
    ...
    combined_dset = CombinedDataset(
        [dset1, dset2], model, model_empty, empty_parameters, costfunction, name="empty"
    )
    assert len(combined_dset.data) == 0  # should have no data

    # Finally, check that info from parameters is being respected by the Iminuit object
    # check that only the requested fit values are keys in the iminuit object
    assert list(combined_dset.costfunction._parameters.keys()) == [
        "global_S",
        "global_BI",
        "delta_empty",
        "sigma_empty",
